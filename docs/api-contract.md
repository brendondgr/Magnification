...
- `POST /api/jobs/filter` — body (optional): `{"job_ids": [int, ...]}` to scope the pass → re-applies **both** rule sets to the currently visible jobs: the `jobs_config.json` `job_titles` / `description_keywords` filter *and* the active profile's `blocked_companies`, `title_blocklist`, and `keyword_groups`. Backs the New Jobs **Filter** button. Saved jobs are exempt, already-hidden jobs are skipped, and the pass is one-directional (jobs are hidden, never un-hidden). Response: `{"success": true, "checked": <n>, "hidden": <n>}`.
