...
- `GET /api/options/llm` → the OpenAI-compatible endpoint config: `{enabled, base_url, api_key, model, temperature, max_tokens, timeout, disable_thinking}`. `base_url` is expected to include the `/v1` prefix. `disable_thinking` (default `true`) asks a reasoning model to skip its hidden chain-of-thought so it emits the answer instead of exhausting `max_tokens` on reasoning (see `docs/plans/llm-fit-reasoning-exhaustion.md`).
